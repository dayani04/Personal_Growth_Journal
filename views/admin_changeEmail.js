...
router.get('/', (req, res) => {
    res.send(`
        ...
        <nav>
            ...
            <a href="/admin_profile">Back</a>
        </nav>

        <div class="container">
            <form action="/admin_changeEmail" method="POST">
            ...
            ${req.session.verificationCode ? `
                <form action="/admin_changeEmail/verify" method="POST" class="verification-form">
...
router.post('/', async (req, res) => {
    ...
    try {
        const user = await db.collection('admins').findOne({ email: oldEmail });

        if (!user) {
            req.session.error = 'Invalid email or password.';
            return res.redirect('/admin_changeEmail');
        }
        ...
        res.redirect('/admin_changeEmail');
    } catch (err) {
